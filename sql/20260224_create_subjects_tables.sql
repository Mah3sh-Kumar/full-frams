...
-- Policy: Admins have full access to all subjects in their branch
-- Branch isolation is enforced through the classes.branch_id join
CREATE POLICY subjects_admin_full
  ...
  USING (
    ...
    AND EXISTS (
      ...
      AND c.branch_id = (SELECT branch_id FROM public.users WHERE id = auth.uid())
    )
  )
  WITH CHECK (
    EXISTS (
...
CREATE POLICY subjects_student_read
  ...
  USING (
    ...
    AND subjects.class_id = (SELECT class_id FROM public.users WHERE id = auth.uid())
...
-- Policy: Admins have full access to subject-teacher assignments in their branch
-- Branch isolation is enforced through subjects -> classes -> branch_id join
...
CREATE POLICY subject_teachers_student_read
  ...
  USING (
    ...
    AND EXISTS (
      SELECT 1 FROM public.subjects s
      WHERE s.id = subject_teachers.subject_id
      AND s.class_id = (SELECT class_id FROM public.users WHERE id = auth.uid())
...
-- MIGRATION COMPLETE
-- ==============================================================================
-- The academic_years, subjects, and subject_teachers tables are now ready for use.
-- All tables have Row Level Security (RLS) enabled with branch isolation policies.
-- 
-- RLS Policies Summary:
-- - academic_years: Authenticated users can read, only admins can modify
-- - subjects: Admins (branch-isolated), teachers (assigned subjects), students (class subjects)
-- - subject_teachers: Admins (branch-isolated), teachers (own assignments), students (class subjects)
-- 
-- Next steps:
-- 1. Insert initial academic year data
-- 2. Create RPC functions for transaction integrity (create/update/delete operations)
-- 
